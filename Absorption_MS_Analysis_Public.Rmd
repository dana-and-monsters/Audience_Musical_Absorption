...
# File Description 
This file contains the analyses for the revised absorption manuscript (Swarbrick, Martin, et al., 2023). This file contains results from the manuscript which underwent one round of major and one round of minor revisions.  

# Abstract 
Musical absorption is a multi-dimensional phenomenon that has been understood in various ways, but is related to strong, immersive, and transformative musical encounters. For this exploratory paper, we used a phenomenologically and psychologically informed questionnaire to measure audience members’ reports of musical absorption and affective experiences at the MusicLab Copenhagen research concert with the Danish String Quartet (DSQ). 
We aimed to examine the relation between musical absorption and 1) attention, mind-wandering, and senses of transformation; 2) affective phenomena of feeling moved/touched and awe; 3) social context as manipulated with the technological mediation of a livestreamed concert; 4) musical context (Beethoven, Schnittke, and Folk tunes); and 5) motion. There were 91 participants in the live audience and 43 participants in the livestreaming audience who completed questionnaires after each piece in the concert and who had their motion measured through an application that recorded accelerometer data from their smartphones. Drawing on methods from experimental psychology, we found that 1) being ‘absorbed in the music’ was not related to mind-wandering, but it was related to a sense of positive transformation; 2) musical absorption was related to experiences of feeling moved, awe, connectedness, and enjoyment, and to being an “admirer” of the DSQ as well as to being familiar with the music; and 3) motion was an indicator of an embodied experience of musical absorption that varied with social context, as reports of musical absorption were related to more motion in the live audience during the folk tunes and less motion in the livestreaming audience during all pieces. We use these results to engage in a phenomenologically informed and empirically enriched discussion of musical absorption and related affective and attentional dynamics.
...
## Transformation and Feelings
Check the mean and standard deviation of positive and negative transformation and feelings. This is reported in Supplementary Table 1.
```{r}
feelings_transformation<-df.full%>%select(Pt_ID, contains(c("transformed", "positive", "negative")))%>%pivot_longer(!Pt_ID, names_to = c("var", "piece"),names_pattern ="(.*)_(.*)", values_to = "response")%>%
  group_by(piece, var)%>%summarise(m = mean(response, na.rm = TRUE), sd = sd(response, na.rm = TRUE), n = n())%>%pivot_wider(names_from = "var", values_from = c("m", "sd"))

feelings_transformation
```
...
check model parameter estimates to understand the interaction between group and motion on absorption

```{r}
#library(ggplot2)
library(see)
library(parameters)
plot(model_parameters(piece_group_x_qomM))
```
